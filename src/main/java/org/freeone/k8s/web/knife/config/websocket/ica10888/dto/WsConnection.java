...
public class WsConnection implements Runnable {
    ...
    public WsConnection(Map<String, String> stringStringMap, WebSocketSession session) throws KubectlException {
        this.setSession(session);
        ApiClient apiClient = K8sUtils.longTimeOutApiClient(Long.valueOf(stringStringMap.get("k8sId")));
        this.setExec( new Exec(apiClient));
        V1Pod pod = Kubectl.get(V1Pod.class).name(stringStringMap.get("name")).namespace(stringStringMap.get("namespace")).apiClient(apiClient).execute();
        String containerName = Objects.requireNonNull(pod.getSpec()).getContainers().get(0).getName();
        this.setPod(new Pod(stringStringMap.get("name"),stringStringMap.get("namespace"),containerName));
